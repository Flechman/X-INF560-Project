...
__global__ void apply_blur_filter_CUDA(pixel *p_i, pixel *receivedTopPart, pixel *receivedBottomPart, int *end, int size, int threshold, int rank, int height, int width, int heightStart, int heightEnd, int actualWidth, int actualHeight)
{
	int idx, j, k;
...

	{
		...
		for (k = 1 + idx; k < width - 1; k += nbThreads)
		{
			for (j = jBoundDown; j < jBoundUp; j++)
			{
				...
				diff_r = (new[TWO_D_TO_ONE_D(j2, k, width)].r - p_i[TWO_D_TO_ONE_D(j2, k, width)].r);
				diff_g = (new[TWO_D_TO_ONE_D(j2, k, width)].g - p_i[TWO_D_TO_ONE_D(j2, k, width)].g);
				...
				p_i[TWO_D_TO_ONE_D(j2, k, width)].r = new[TWO_D_TO_ONE_D(j2, k, width)].r;
				p_i[TWO_D_TO_ONE_D(j2, k, width)].g = new[TWO_D_TO_ONE_D(j2, k, width)].g;
				p_i[TWO_D_TO_ONE_D(j2, k, width)].b = new[TWO_D_TO_ONE_D(j2, k, width)].b;
			}
		}
		err = cudaThreadSynchronize();
		if(err != cudaSuccess) {
...
void apply_blur_filter(animated_gif *image, int size, int threshold, int rank, int nbProc)
{
	...
	for (i = 0; i < image->n_images; i++)
	{
		...
		/* Allocate array of new pixels + send and receive buffers*/
		...
		pixel *d_receivedTopPart, *d_receivedBottomPart, *d_p_i;
		int *d_end;
...
void apply_sobel_filter(animated_gif *image, int rank, int size, int blur_radius)
{
	...
	for (i = 0; i < image->n_images; i++)
	{
		int sizeToUse = compute_nb_proc_to_use(size, image->actualHeight[i], blur_radius);
...
int main(int argc, char **argv)
{
	...
	double duration, duration_loaded, duration_export, duration_filters;
	int image_counter;
	...
	image_counter = atoi(argv[3]);
	nb_threads = argv[4] != NULL ? atoi(argv[4]) : 1;

	if (provided < MPI_THREAD_FUNNELED)
	{
		printf("Not enough threads for OMP");
